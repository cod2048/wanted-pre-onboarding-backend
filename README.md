...
npm test
```
테스트 실행 전 데이터베이스의 데이터를 모두 삭제 후 실행하길 권장합니다.
### 테스트별 상세 설명
- 총 8개 파일
    - 회사 생성
    - 채용공고 생성
...
- 37개 test
  - 회사 생성
    - 올바른 회사데이터 전송 시 statusCode(201)반환 확인
    - 필수 데이터 누락 후 전송 시 statusCode(500)반환 확인
  - 채용공고 생성
    - 올바른 채용공고데이터 전송 시 statusCode(201)반환 확인
    - 필수 데이터 누락 후 전송 시 statusCode(500)반환 확인
  - 채용공고 수정
    - 보낸 수정 데이터와 반환받은 수정된 데이터가 일치하는 지, statusCode(201) 확인
    - 없는 채용공고를 업데이트하려고 시도 시 statusCode(404)반환 확인
  - 채용공고 삭제
    - 정상 삭제 시 statusCode(204) 확인
    - 없는 채용공고 삭제 시 statusCode(404) 확인
  - 채용공고 목록 조회
    - 정상 조회 시 statusCode(200) 확인
  - 채용공고 검색 및 상세페이지 조회
    - statusCode(200)과 미리 넣어놓은 더미데이터의 갯수가 맞는지 확인
    - 채용공고 상세페이지 정상조회 시 statusCode(200) 확인
    - 없는 채용공고 목록 조회 시 statusCode(404) 확인
  - 유저 생성
    - 정상 생성 시 statusCode(201) 확인
    - 필수 정보 누락 시 statusCode(500) 확인
  - 채용공고 지원
    - 정상 지원 시 statusCode(201) 확인
    - 중복 지원 시 statusCode(403) 확인
    - 없는 채용공고 or 없는 유저 아이디일 경우 statusCode(500) 확인
### 테스트 실행 결과
